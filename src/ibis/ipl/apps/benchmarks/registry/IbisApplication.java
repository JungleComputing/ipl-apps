...
final class IbisApplication implements Runnable, RegistryEventHandler {
	...
	private Ibis join() throws IbisCreationFailedException {
		IbisCapabilities s = new IbisCapabilities(IbisCapabilities.MEMBERSHIP, IbisCapabilities.ELECTIONS, IbisCapabilities.SIGNALS);
		PortType p = new PortType(PortType.CONNECTION_ONE_TO_ONE,
			...
			while (true) {
				...
				while (!left) {
					...
					case 2:
						...
						IbisIdentifier id2 = ibis.registry().getElectionResult(
								"bla");
						break;
					case 3:
						logger.debug("doing getElectionResult with timeout");
						IbisIdentifier id3 = ibis.registry().getElectionResult(
								"bla", 1000);
